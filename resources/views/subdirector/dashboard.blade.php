...
<div class="flex flex-col min-h-screen bg-gray-50">
  ...
  <div class="bg-white border-b border-gray-200 px-6 py-3 flex items-center justify-between sticky top-0 z-10">
    <div>
      <h1 class="text-base font-semibold text-gray-800">Panel del Subdirector</h1>
  ...
  <div class="flex-1 p-6">
    ...
    <form method="GET" action="{{ route('subdirector.dashboard') }}" class="flex flex-wrap gap-2 mb-4">
      ...
      <select name="tipo" class="border border-gray-200 rounded-xl px-3 py-2 text-sm bg-white">
      ...
      <select name="estado" class="border border-gray-200 rounded-xl px-3 py-2 text-sm bg-white">
      ...
      <table class="w-full text-sm">
        ...
        <tbody class="divide-y divide-gray-50">
          ...
          <tr class="hover:bg-gray-50 transition {{ $r->anulado ? 'opacity-50' : '' }}">
            ...
            <td class="px-5 py-3">
              <span class="text-xs px-2 py-0.5 rounded-full font-medium {{ $r->tipo === 'queja' ? 'bg-red-50 text-red-600' : 'bg-teal-50 text-teal-700' }}">
                {{ ucfirst($r->tipo) }}
              </span>
            </td>
            ...
            <td class="px-5 py-3 text-xs text-gray-400 tabular-nums">{{ \Carbon\Carbon::parse($r->fecha)->format('d/m/Y') }}</td>
            <td class="px-5 py-3">
              <a href="{{ route('subdirector.detalle', $r->id) }}" class="text-xs text-[#01696f] hover:underline font-medium">
                Ver detalle
              </a>
            </td>
          </tr>
          @empty
          <tr>
            <td colspan="6" class="px-5 py-16 text-center text-gray-400 text-sm">No hay registros aún.</td>
          </tr>
          @endforelse
        </tbody>
      </table>
